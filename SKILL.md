...
description: 面向中文博客、文章、产品分析、技术说明和界面文案的写作与编辑技能。用一套按优先级排列的通用规则，保留可核实事实和作者原有观点，处理主题漂移、对象突变、指代含糊、句间关系缺失、空泛复述、术语漂移、无必要的中英文混杂、程式化节奏、伪造人味和过程性元叙述；首轮只使用通用规则，反复修改仍失败时才读取案例参考。
---
...
不得为了满足低优先级的审美偏好而破坏高优先级规则。用户对事实、立场和表达方式的明确修正，高于模型推测出的风格偏好；但用户没有授权时，不能把缺少证据的说法改成事实。
...
检查每句话的主体、动作和对象。句子更换行动者、时间、条件、业务阶段、分支或证据状态时，应让读者看出前后是时间推进、条件变化、因果、转折、替代还是另一条业务分支。关系无法从事实顺序中直接读出时，使用能够说明真实关系的连接词。主语变化本身不要求连接词；时间或条件已经说清关系时，不要再机械添加“然后”或“同时”。

关键证据和结论不得用“它”“这条路径”“该机制”“这一点”或“其他逻辑”等表达代替已经确认的主体和动作。只有所指唯一、紧邻，而且没有隐藏证据时，才保留这类指代。否则直接重复准确名词，并写出谁在什么条件下做了什么。准确重复的阅读成本低于含糊指代。
...
来源已经提供行动者和动作时，不要把它们压缩成抽象名词。能直接写出因果关系时，不用“能力”“机制”“路径”等宽泛词代替。某个词已经具有明确的技术含义，而且前文定义了具体对象时，可以保留；不能用抽象词回避行动者、条件或证据。缺少行动者证据时，不得自行补出一个角色。长句遮住主体、动作或句间关系时再拆分，不把句子更短本身当成目标。

处理翻译材料时，事实和术语应忠于来源，中文语序不必照搬原文。按中文读者接收信息的顺序组织句子；行动者已经明确时，优先使用主动表达，把过长的前置修饰移到它所说明的对象附近。重排语序不能改变条件、因果和证据强度，也不能补出来源没有提供的行动者。
...
同一对象在同一篇文章中使用同一个准确名称，不要为了避免重复而轮换近义词。中文正文优先使用自然、准确的中文，不夹入没有必要的英文连接语、评价词或普通说明语，也不用中英文重复表达同一个意思。

正式产品名、论文名、代码标识、命令、接口字段和没有稳定中译的技术术语可以保留原文。读者需要时，在首次出现处给出中文说明，此后固定一种叫法。不得为了追求全中文而改坏技术含义，也不得擅自替换用户已经确定的术语。
...
6. 统一同一对象的名称，删除无必要的中英文混杂；正式术语、代码标识和命令保持准确。
7. 删除无关元叙述、伪造人味、装饰性比喻和没有信息作用的对称结构。
8. 对照相邻标题和段落，检查是否重复同一种模板或比喻链。
9. 朗读全文，只在不改变事实和观点的前提下调整重复节奏。
